...
function Filter({
  ...
  const [landWidthValue, setLandWidthValue] = useState(
    filters.landWidthValue || ""
  );
  const handleLandWidthChange = (event) => {
    setLandWidthValue(event.target.value);
    setFilters((prevFilters) => ({
  ...
  const [landDepthValue, setLandDepthValue] = useState(
    filters.landDepthValue || ""
  );
  const handleLandDepthChange = (event) => {
    setLandDepthValue(event.target.value);
    setFilters((prevFilters) => ({
  ...
  const [bedroomActive, setBedroomActive] = useState(
    filters.bedroomActive || 0
  );
  const handleBedRoomChange = (num) => {
    setBedroomActive(num);
    setFilters((prevFilters) => ({
  ...
  const [driveWayValue, setDriveWayValue] = useState(
    filters.driveWayValue || "Right"
  );
  const handleDriveWayChange = (event) => {
    setDriveWayValue(event.target.value);
    setFilters((prevFilters) => ({
  ...
  useEffect(() => {
    if (active === "displayCenter") {
      ...
      return () => {
        // Clean up event listeners
        mediaQueryButton.removeEventListener(
          "change",
          handleButtonMediaQueryChange
        );
      };
    }
  }, [active]);
  ...
  //     const setHeight = () => {
  //       const sidePopUp = document.querySelector(`.${styles.bottomDiv}`);
  //       if (sidePopUp) {
  //         if (isPhone) {
  //           sidePopUp.style.height = "calc(100vh - 443px)";
  //           if (isNotStandalone) {
  //             sidePopUp.style.height = "calc(100vh - 386px)";
  //           }
  //         } else {
  //           sidePopUp.style.height = "calc(100vh - 386px)";
  //         }
                ...
                >
                  ...
                  <div>
                    {isEditable.min ? (
                      ...
                      <span>
                        {sliderValue[0] === 0 ? "0" : sliderValue[0]} sq
                      </span> // Display value when not editable
                    )}
                  </div>
                </div>
                ...
                >
                  ...
                  <div>
                    {isEditable.max ? (
                      ...
                      <span>
                        {sliderValue[1] === 0 ? "0" : sliderValue[1]} sq
                      </span> // Display value when not editable
                    )}
                  </div>
                </div>
