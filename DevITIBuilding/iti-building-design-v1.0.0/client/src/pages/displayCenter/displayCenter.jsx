    ...
    useEffect(()=>{
        ...
        if(projects?.length > 0 && searchValue !== ''){
            ...
            const filtered = projects.filter(project => 
                project?.custom_fields?.find(field => field?.name?.toLowerCase() === "name")?.value?.toLowerCase().includes(searchValue.toLowerCase())
            );
            ...
            const filterProjects = () => {
                ...
                return filteredProjects.filter(project => {
                    ...
                    if (filters?.sliderValue) {
                        ...
                        conditions.push((fieldValue * 0.1076391041671).toFixed(2) >= sliderMin && (fieldValue * 0.1076391041671).toFixed(2) <= sliderMax);
                    }
            ...
            <div className={styles.mainDiv}>
                <div className={styles.topbarDiv}>
                    <TopBar 
                        ...
                        setFilterPopUp={setFilterPopUp}
                        setFilterEnable={setFilterEnable}
                        closeActive={buttonDis}
                        setCloseActive={setButtonDis}
                        setFilters={setFilters}
                    />
                </div>
                <ScrollBar>
...
<>
    ...
    }
    {
        finalActiveProjects.length === 0 && likePlansActive && buttonDis && searchValue === '' &&
            <div>
                No projects yet! Start bookmarking projects to see them here.
            </div>
    }
    {
        finalActiveProjects.length === 0 && searchValue === "" && !loading && buttonDis && !likePlansActive &&
