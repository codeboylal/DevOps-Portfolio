      ...
      if (req?.body?.history_items?.[0]?.parent_id) {
        ...
        if (model && model.length > 0) {
          ...
          if (taskDetails) {
            ...
            //   }
            if (taskDetails.custom_item_id === 1004 || item.custom_item_id !== 1005) {
            ...
            if (assigneeEmail) {
              ...
              if (!profile) {
                return res
                  .status(200)
                  .json(
                    ResponseHandler(404, null, "No User Found to Assign Task")
                  );
              }
              ...
              const parentTaskResponse = await fetch(
                // `https://api.clickup.com/api/v2/task/${task_Id}?include_subtasks=true&include_closed=true`,
                `https://api.clickup.com/api/v2/task/${taskDetails.top_level_parent}`,
                {
                  method: "GET",
                  headers: {
                    "Content-Type": "application/json",
