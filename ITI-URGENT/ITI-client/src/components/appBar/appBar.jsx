...
function AppBar({ name = "", sideBarEnable, active, searchValue, handleSearchChange, setLikePlansActive, likePlansActive = false, handleLikePlans, }) {
    ...
    const [nameDB, setNameDB] = useState(name || sessionStorage.getItem("name"))

    const words = nameDB?.split(' ');
    const truncatedName = words?.slice(0, 9).join(' ') + (words?.length > 9 ? '...' : '');
    ...
    return (
        <div className={styles.appDiv}>
            <div className={styles.sidebarSearch}>
                ...
                >
                    <MenuIcon style={{
                        ...
                        display: active === "terms" ? 'none' : active === "help" ? 'none' : ''
                    }} />
                    <svg style={{
                        cursor: 'pointer',
                        display: !(active === "help" || active === "terms") && 'none'
                    }} onClick={() => {
                        navigate("/settings")
                    }} width="16" height="20" viewBox="0 0 16 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M14.2499 27.75C14.5696 27.75 14.8896 27.6278 15.1337 27.3837C15.6221 26.8953 15.6221 26.1043 15.1337 25.6162L3.51742 14L15.1337 2.38371C15.6221 1.89527 15.6221 1.10434 15.1337 0.616211C14.6452 0.128086 13.8543 0.127773 13.3662 0.616211L0.866172 13.1162C0.377734 13.6046 0.377734 14.3956 0.866172 14.8837L13.3662 27.3837C13.6102 27.6278 13.9302 27.75 14.2499 27.75Z" fill="black" />
                    </svg>
                </div>

                <div className={cx(styles.heading, styles.active)}>
                    {
                        active==="displayCenter" ? "Display Center":
                        active==="settings"? "Settings":
                        active==="terms"?"Terms & Conditions":
                        active==="help"?"Help":
                        active==="admin"?"Admin Panel":
                        active==="sales"?"Sales":
                        active==="projects"?"Projects":
                        active==="inbox"?"Inbox":
                        active==="Constructor"?"Constructor":
                        "Dummy Text"
                    }
                </div>
                <div
                ...
                <div
                    style={{
                        display: active === "terms" ? 'flex' : active === "help" ? "flex" : "none"
                    }}
                    className={
                        cx(styles.backSpan, styles.displayNone)
                    }
                >
            ...
            <div
                className={cx(styles.iconsDiv, styles.displayNone)}
            >
                ...
                <div className={styles.profile}>
                    ...
                    {isTruncated ? (
                        ...
                        >
                            <label className={styles.truncatedLabel}>
                                {truncatedName}
                            </label>
                        </Tooltip>
                    ) : (
            ...
            {
                sidePopUp && <SidebarPopUp PopUp={sidePopUp} setSidePopUp={setSidePopUp} active={active} />
            }
        </div>
    )
}
